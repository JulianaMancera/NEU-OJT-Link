...
const WeeklyJournalReport = ({ isOpen, onClose }: WeeklyJournalReportProps) => {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const { error } = await supabase.storage.from("journals").upload(filePath, file);
      if (error) {
      ...
      // Get public URL
      const { data: fileData } = supabase.storage.from("journals").getPublicUrl(filePath);
  ...
  return (
    <div className={`fixed inset-0 bg-opacity-50 backdrop-blur-sm flex items-center justify-center ${isOpen ? "visible" : "invisible"}`}>
      ...
      <div className="relative bg-gray-200 shadow-xl rounded-lg p-6 border-2 border-black-400 w-full max-w-2xl z-50">
        <h2 className="text-2xl font-semibold text-gray-800 text-center mb-4">UPLOAD WEEKLY JOURNAL REPORT</h2>
...
export default WeeklyJournalReport;

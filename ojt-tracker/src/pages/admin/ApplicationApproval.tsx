      ...
      folders.map(async (folder) => {
        if (folder === "endorsement-letter") {
          ...
          const { data: files, error: listError } = await supabase.storage
            ...
            .list(path);
          if (listError || !files) { out[folder] = []; return; }
  ...
  return (
    <AdminLayout>
      <div className="mt-24 bg-[#FFFCF9] border border-black rounded-lg p-6 max-w-8xl mx-auto text-black shadow-lg">
        <h2 className="text-center py-4 font-bold text-5xl mb-6">Application Approvals</h2>
                ...
                >
                  ...
                  <td className="p-3 text-center">
                    <select
                      ...
                      className="border p-2 rounded-md bg-white shadow-sm focus:ring-2 focus:ring-blue-500 w-full"
                      disabled={loading}
                    >
      ...
      {showModalFor && (
        <div
          className="fixed inset-0 bg-gradient-to-b from-[#3657DB]/75 to-[#8D95B5]/75 flex items-center justify-center z-50"
